...
document.addEventListener("DOMContentLoaded", () => {
    ...
    function appendFormOutput(outputId, message) {
        const outputDiv = document.getElementById(outputId);
        if (!outputDiv) return;
        outputDiv.innerHTML += `<div class="output-entry">${message}</div>`;
    }
    ...
    if (arithmeticForm) {
        arithmeticForm.addEventListener("submit", (event) => {
            ...
            const firstNumber = getNumber("arithmeticFirstNumber");
            const secondNumber = getNumber("arithmeticSecondNumber");
            const operatorInput = document.querySelector('input[name="arithmeticOperator"]:checked');
            const operator = operatorInput ? operatorInput.value : "add";
            ...
            const message = `${firstNumber} ${symbol} ${secondNumber} = ${result}`;
            alert(message);
            appendFormOutput("arithmeticOutput", escapeHTML(message));
        });
    }
    ...
    if (factorialForm) {
        factorialForm.addEventListener("submit", (event) => {
            ...
            const n = getNumber("factorialNumber");
            ...
            if (n > 50) {
                alert("Please enter an integer from 0 through 50 so the result remains readable.");
                return;
            }
            ...
            alert(message);
            appendFormOutput("factorialOutput", escapeHTML(message));
        });
    }
    ...
    if (fibonacciForm) {
        fibonacciForm.addEventListener("submit", (event) => {
            ...
            const n = getNumber("fibonacciNumber");
            ...
            if (n > 500) {
                alert("Please enter an integer from 0 through 500 so the result remains readable.");
                return;
            }

            const result = fibonacci(n).toString();
            const message = `Fibonacci number F(${n}) = ${result}`;
            alert(message);
            appendFormOutput("fibonacciOutput", escapeHTML(message));
        });
    }
    ...
    if (rangeForm) {
        rangeForm.addEventListener("submit", (event) => {
            ...
            const numbers = [
                getNumber("rangeFirstNumber"),
                getNumber("rangeSecondNumber"),
                getNumber("rangeThirdNumber")
            ];
            ...
            const message = `Maximum: ${maximum}, Minimum: ${minimum}, Range: ${range}`;
            alert(message);
            appendFormOutput("rangeOutput", escapeHTML(message));
        });
    }
    ...
    if (mailingListForm) {
        mailingListForm.addEventListener("submit", (event) => {
            ...
            if (!firstName || !lastName || !email || !zipCode) {
                alert("Please complete every mailing list field.");
                return;
            }
            ...
            document.getElementById("mailingListOutput").innerHTML += outputString;
